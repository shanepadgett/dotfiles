...
uninstall_brew_packages() {
  ...
  if [[ -f $brewfile ]]; then
    ...
    local brew_packages
    brew_packages=$(grep '^brew ' "$brewfile" | sed 's/brew "\([^"]*\)".*/\1/' | tr '\n' ' ')
    local cask_packages
    cask_packages=$(grep '^cask ' "$brewfile" | sed 's/cask "\([^"]*\)".*/\1/' | tr '\n' ' ')
...
restore_backups() {
  ...
  local latest_backup
  latest_backup=$(find "$HOME" -maxdepth 1 -type d -name ".config-backup-20*" 2>/dev/null | sort -r | head -n1)
...
cleanup_directories() {
  ...
  if [[ -d "$HOME/Downloads" ]]; then
    ...
    download_count=$(find "$HOME/Downloads" -mindepth 1 -maxdepth 1 | wc -l)
